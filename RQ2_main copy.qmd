...
## Model 2: Without Religion
```{r echo=FALSE, results=FALSE}
model_without_religion <- glm(ImportanceOfStoppingDiscriminationAgainstMinorities ~ Gender +
               TreatedUnfairlyOrDiscriminatedAgainst + 
               DiscriminationImpactOnLifeSatisfaction +
               PerceptionsOfDiscriminationAgainstWhites + 
               PerceptionsOfDiscriminationAgainstBlacks +
...
# Check multicollinearity using VIF
vif(model_without_religion)
```

```{r echo=FALSE, results=FALSE}

# Extract odds ratios
odds_ratios <- exp(coef(model_without_religion))

# Get confidence intervals for coefficients
conf_intervals <- exp(confint(model_without_religion))

# Extract p-values
p_values <- summary(model_without_religion)$coefficients[, 4]

# Combine everything into a single data frame
results_model_without_religion <- data.frame(
  OddsRatio = odds_ratios,
  LowerCI = conf_intervals[, 1],
  UpperCI = conf_intervals[, 2],
  PValue = p_values
...
# Display results
results_model_without_religion
```


## Model 3: Model without religion + interaction term
```{r echo=FALSE, results=FALSE}
model_noreligion_Interaction <- glm(ImportanceOfStoppingDiscriminationAgainstMinorities ~ Gender +
               TreatedUnfairlyOrDiscriminatedAgainst + 
               DiscriminationImpactOnLifeSatisfaction +
               PerceptionsOfDiscriminationAgainstWhites + 
               PerceptionsOfDiscriminationAgainstBlacks +
...
# Check multicollinearity using VIF
vif(model_noreligion_Interaction)
```
...
# Extract odds ratios
odds_ratios <- exp(coef(model_noreligion_Interaction))

# Get confidence intervals for coefficients
conf_intervals <- exp(confint(model_noreligion_Interaction))

# Extract p-values
p_values <- summary(model_noreligion_Interaction)$coefficients[, 4]

# Combine everything into a single data frame
results_model_noreligion_Interaction <- data.frame(
  OddsRatio = odds_ratios,
  LowerCI = conf_intervals[, 1],
  UpperCI = conf_intervals[, 2],
  PValue = p_values
...
# Display results
model_noreligion_Interaction
```


```{r}
results_model_noreligion_Interaction
```
...
```{r}
model_noreligion_Interaction
model_without_religion

anova(model_without_religion, model_noreligion_Interaction, test="Chisq")
...
```{r}
# Convert the prediction to a factor with correct levels
...
## Figure 1: Proportion of outcome variable for RQ 1
