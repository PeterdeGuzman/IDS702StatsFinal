...
```{r}
load("/Users/jenniferli/Desktop/2020cmps_clean2.RData")
```
...
rename_map <- c(
    ...
    "race_recodedAIAN" = "Race - Native", # why use Native instead of AIAN?
...
# Plot with Andrea's following comments all considered:

1.  make sure to use appropriate variable labels.
2.  I see that you ordered them based on the OR values, but I think it’s confusing to see the responses to the questions in different places. I would still group the questions together, and maybe put them in descending order within each question?
3.  Also make sure the reference levels are clear so that the reader can appropriately interpret the plot
...
odds_ratio_plot_labelled <- function(model) {
  ...
  sort_by_group_and_name <- function(x) {
    # Split the strings to get group numbers and names
    groups <- as.numeric(sub("^(\\d+)\\..+$", "\\1", x))
    # Get the part after the colon for sorting within groups
    names <- sub("^\\d+\\.[^:]+: (.+) \\(ref.+$", "\\1", x)
    ...
    # Sort by group (ascending) and then by name (descending)
    sort_df <- sort_df[order(sort_df$group, -xtfrm(sort_df$name)), ]
    return(sort_df$original)
  }
  
  # Apply the custom sorting
  sorted_names <- sort_by_group_and_name(rownames(or_data_sig_renamed))
  or_data_sig_renamed <- or_data_sig_renamed[sorted_names, ]
  
  ggplot() +
    geom_vline(xintercept = 1, linetype = "dashed", color = "red", size = 0.7) +
    geom_point(aes(x = or_data_sig_renamed[,1], 
                   y = factor(rownames(or_data_sig_renamed), 
                          levels = rev(rownames(or_data_sig_renamed)))), 
              size = 4, color = "blue") +
    geom_errorbarh(aes(xmin = or_data_sig[,2], 
                       xmax = or_data_sig[,3],
                       y = factor(rownames(or_data_sig_renamed), 
