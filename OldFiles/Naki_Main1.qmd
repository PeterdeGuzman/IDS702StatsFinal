...
# Recode 'Don' as 'Don't know'
dataset_new[perception_cols] <- lapply(dataset_new[perception_cols], function(x) {
  x <- ifelse(x == "Don", "Don't know", x)  # Recode 'Don' to 'Don't know'
  return(x)
})
...
```{r}
predicted_classes <- ifelse(predicted_probabilities > 0.7, "Not important", "Important")
actual_classes <- dataset_clean$ImportanceOfStoppingDiscriminationAgainstMinorities
```
...
```{r}
library(caret)
confusion <- confusionMatrix(factor(predicted_classes), factor(actual_classes))
print(confusion)
```
