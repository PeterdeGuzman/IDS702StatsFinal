...
```{r}
newdata <- dataset_clean[, c("ImportanceOfStoppingDiscriminationAgainstMinorities", 
                             "PrimaryRace", "Gender", "HighestLevelOfEducation", "TreatedUnfairlyOrDiscriminatedAgainst",
                             "DiscriminationImpactOnLifeSatisfaction", "PerceptionsOfDiscriminationAgainstWhites",
                             "PerceptionsOfDiscriminationAgainstBlacks", "PerceptionsOfDiscriminationAgainstAsianAmericans",
                             "PerceptionsOfDiscriminationAgainstNativeAmericans", "PerceptionsOfDiscriminationAgainstLatinos")]
```

```{r}
actual_data <- dataset_clean$ImportanceOfStoppingDiscriminationAgainstMinorities
predicted_probs <- predict(model_stepwise, newdata = newdata, type = "response")
predicted_probs
```

```{r}
library(pROC)
roc_curve <- roc(actual_data, predicted_probs)  # predicted_probs = probabilities for the positive class
plot(roc_curve)
auc(roc_curve)
```